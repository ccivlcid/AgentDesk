...
검토일: 2026-03-09  
대상: `claw-empire-differentiation-plan.md`, `agentdesk-2.0-menu-spec.md`  
...
| 주제 | differentiation plan | menu spec | 일치 |
|------|------------------------|-----------|------|
| CEO 노출 제거 | 직책(CEO/CTO) 제거, Phase 1 용어 전환 | Top에서 CEO/ceoName 제거, 로고+프로젝트 셀렉터만 | ✅ |
| 첫 화면/엔트리 | 프로젝트·카테고리 선택/생성 | 앱 런치 후 (선택) 프로젝트/카테고리 컨텍스트 → 대시보드 | ✅ |
| 카테고리 에디터 | Phase 2 카테고리 CRUD·에디터 | 설정 탭 또는 별도 서브메뉴 | ✅ |
| 대시보드 | 4분면(목표/리스크/게이트/산출물) | 2.0 "홈", 4 quadrants | ✅ |
...
| 3 | 카테고리 선택형 프로젝트 생성 플로우 (UI) | 1, 2 | 생성 시 카테고리 선택 → 버전 고정·기본값 주입 |
| 4 | 카테고리 에디터 (정책/KPI/게이트/산출물) | 1 | 설정 탭 또는 별도 뷰 |
| 5 | First-entry 플로우 (프로젝트 없을 때) | 3 | “새 프로젝트” or “카테고리 선택 후 생성” |
...
- **menu spec**: As-Is가 코드와 일치하고, To-Be가 전략과 맞으며 구현 체크리스트가 있어 개발에 바로 쓸 수 있음. 문자 인코딩과 Phase 2 크로스레퍼런스만 정리하면 됨.
- **진행 순서**: 문서·기획 정리 후 Phase 1(스키마·시드 → 메뉴/브랜드 → 용어), Phase 2(카테고리 API → 프로젝트 모델 → 생성 플로우 → 에디터) 순으로 진행하면 의존성을 줄일 수 있음.
- 두 문서는 서로 모순 없이 맞춰져 있어, 현재 상태로도 리뉴얼 기획·개발 착수에 무리가 없습니다.
