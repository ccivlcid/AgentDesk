...
- 페르소나 단계 레이블: "이 에이전트는 어떤 **방식**으로 생각할까요?" (선택 사항)
- 선택 없이 다음 진행 가능 → 기본 에이전트 생성

### 4-2. 방식 선택 UI — PersonaCatalog ("방식 우선" 카드)

카드는 **방식 키워드를 헤더**로, 인물 이름을 보조 텍스트로 표시:

```
┌─ 사고 방식 선택  ·  선택 사항 ─────────────────────────────┐
│                                                            │
│  [전체]  [단순함·완성도]  [1st 원칙]  [가치투자]  [직접화법]  │
│                                                            │
│  ┌──────────────────┐  ┌──────────────────┐              │
│  │ 단순함에 집착한다  │  │ 1st 원칙으로      │              │
│  │ 사용자 경험 우선   │  │ 불가능을 가능하게  │              │
│  │                  │  │                  │              │
│  │ Steve Jobs 방식   │  │ Elon Musk 방식    │              │
│  │ [제품 기획 · UX]  │  │ [기술 · 혁신 전략] │              │
│  └──────────────────┘  └──────────────────┘              │
│                                                            │
│  > 선택됨: Steve Jobs 방식                                  │
│    제품 기획, UX 리뷰, 브랜딩에 적합합니다                    │
└────────────────────────────────────────────────────────────┘
```

카드 구조 (위→아래):
1. **방식 키워드** (큰 텍스트): 사고 방식의 핵심 원칙 1~2줄
2. **인물명 방식** (보조 텍스트): "Steve Jobs 방식"
3. **적합한 태스크** (태그): `[제품 기획]` `[UX]`
...
- 태그: `[제품 기획]`, `[UX]` — `status-badge` 패턴 (JetBrains Mono)
- 방식 키워드: Sora Bold / 인물명: JetBrains Mono 보조색
- 필터 탭: `terminal-section-header` 스타일
...
│  🤖  Claude-Dev                         │
│      Senior Developer                   │
│      [RUNNING]  [단순함]                 │  ← 방식명 배지 (인물명 아님)
│                                         │
│  "단순함에 집착하는 방식 적용 중"          │  ← 방식 설명 서브텍스트
└─────────────────────────────────────────┘
```
...
- 디자인 시스템: `docs/design/design-retro-terminal-overhaul.md`
