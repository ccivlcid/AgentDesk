<!-- BEGIN agentdesk orchestration rules -->
...
- KO: `✅ AgentDesk 업무지시 전달 완료` (회의 진행) / `✅ AgentDesk 업무지시 전달 완료 (회의 생략)` (회의 없이)
...
2. Show the target AGENTS path (`~/.agentdesk/workspace/AGENTS.md` by default, or response field).
3. Ask the user for installation consent in the same language:
   - KO: `AgentDesk AGENTS가 구버전입니다(HTTP 428). 최신 규칙 설치가 필요합니다. 설치 파일 경로를 안내드렸습니다. 지금 제가 직접 설치해드릴까요? (예/아니오)`
   - EN: `AgentDesk AGENTS is outdated (HTTP 428). Latest rules must be installed. I listed installer paths. Should I install it now? (yes/no)`
...
The AgentDesk server detects the `$` prefix and automatically:
...
1. **`$` directive**: After collecting required meeting/path inputs and sending to API, reply with only `✅ AgentDesk 업무지시 전달 완료` (or language equivalent). Nothing else.
