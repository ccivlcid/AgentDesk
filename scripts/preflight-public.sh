...
echo "== AgentDesk public release preflight =="
...
blocked_tracked=(
  ...
  ".env.production"
  "agentdesk.sqlite"
  "agentdesk.sqlite-shm"
  "agentdesk.sqlite-wal"
)
...
if pnpm run build >/tmp/agentdesk-preflight-build.log 2>&1; then
  pass "Build succeeded"
else
  fail "Build failed (see /tmp/agentdesk-preflight-build.log)"
  tail -n 80 /tmp/agentdesk-preflight-build.log || true
fi
