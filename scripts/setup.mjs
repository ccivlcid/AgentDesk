...
function resolveWorkspaceDir() {
  // Try reading workspace from agentdesk.json
  ...
  // Check AGENTDESK_PROFILE
  const profile = process.env.AGENTDESK_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    const profdir = path.join(os.homedir(), ".agentdesk", `workspace-${profile}`);
...
function findAgentsPath() {
  ...
  // Default target: current project root (agentdesk users first).
  // AgentDesk workspace targeting should be explicit via --agents-path.
...
function main() {
  ...
  console.log(`[AgentDesk] Setting up orchestration rules`);
  console.log(`[AgentDesk] Target: ${agentsPath}`);
  console.log(`[AgentDesk] Port: ${port}`);
  ...
  if (existingContent.includes(START_MARKER) && existingContent.includes(END_MARKER)) {
    ...
    console.log(`[AgentDesk] Updated existing orchestration rules in ${agentsPath}`);
    console.log(`[AgentDesk] Done!`);
    return;
  }
  ...
  fs.writeFileSync(agentsPath, newContent, "utf8");
  console.log(`[AgentDesk] Orchestration rules added to top of ${agentsPath}`);
  console.log(`[AgentDesk] Your existing AGENTS.md content is preserved below.`);
  console.log(`[AgentDesk] Done!`);
}
