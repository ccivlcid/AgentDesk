...
export function applyDefaultSeeds(db: DbLike): void {
  ...
  if (agentCount === 0) {
    ...
    // Development (4)
    insertAgent.run(randomUUID(), "Ada Lovelace", "에이다 러브레이스", "dev", "team_leader", "claude", "👩‍💻", "세계 최초의 프로그래머, 꼼꼼한 알고리즘 설계자");
    insertAgent.run(randomUUID(), "Alan Turing", "앨런 튜링", "dev", "senior", "codex", "🧮", "컴퓨터 과학의 아버지, 논리적 문제 해결의 천재");
    insertAgent.run(randomUUID(), "Nikola Tesla", "니콜라 테슬라", "dev", "junior", "copilot", "⚡", "창의적 발명가, 미래를 앞서간 혁신가");
    insertAgent.run(randomUUID(), "Andrej Karpathy", "안드레이 카파시", "dev", "senior", "claude", "🤖", "AI/딥러닝 개발의 선구자");
    // Design (3)
    insertAgent.run(randomUUID(), "Leonardo da Vinci", "레오나르도 다 빈치", "design", "team_leader", "claude", "🖼️", "르네상스 만능인, 예술과 공학의 융합 대가");
    insertAgent.run(randomUUID(), "Frida Kahlo", "프리다 칼로", "design", "junior", "gemini", "🌺", "감성적이고 독창적인 예술가");
    insertAgent.run(randomUUID(), "Steve Jobs", "스티브 잡스", "design", "senior", "claude", "🍏", "혁신적인 제품 디자인 비전가");
    // Planning (2)
    insertAgent.run(randomUUID(), "Zhuge Liang", "제갈량", "planning", "team_leader", "codex", "🪶", "천하삼분지계의 전략가, 신출귀몰한 기획의 달인");
    insertAgent.run(randomUUID(), "Machiavelli", "마키아벨리", "planning", "senior", "claude", "📜", "군주론의 저자, 냉철한 전략 분석가");
    // Operations (3)
    insertAgent.run(randomUUID(), "Genghis Khan", "칭기즈 칸", "operations", "team_leader", "claude", "🏇", "역사상 가장 효율적인 제국 운영자");
    insertAgent.run(randomUUID(), "James Watt", "제임스 와트", "operations", "senior", "codex", "🔧", "산업혁명의 아버지, 자동화의 선구자");
    insertAgent.run(randomUUID(), "Bill Gates", "빌 게이츠", "operations", "senior", "codex", "💼", "글로벌 소프트웨어 제국 운영의 달인");
    // QA/QC (3)
    insertAgent.run(randomUUID(), "Marie Curie", "마리 퀴리", "qa", "team_leader", "claude", "⚗️", "노벨상 2회 수상, 철저한 실험과 검증의 대가");
    insertAgent.run(randomUUID(), "Isaac Newton", "아이작 뉴턴", "qa", "senior", "codex", "🍎", "자연법칙의 발견자, 정밀한 분석과 검증의 거장");
    // DevSecOps (2)
    insertAgent.run(randomUUID(), "Sun Tzu", "손자", "devsecops", "team_leader", "claude", "⚔️", "손자병법의 저자, 최고의 방어 전략가");
    insertAgent.run(randomUUID(), "Hedy Lamarr", "헤디 라마", "devsecops", "senior", "codex", "📡", "주파수 도약 기술 발명가, 보안 통신의 선구자");
    // QA Junior (1)
    insertAgent.run(randomUUID(), "Galileo Galilei", "갈릴레오 갈릴레이", "qa", "junior", "gemini", "🔭", "모든 것을 의심하고 관찰하는 탐구자");
  ...
  // Migrate: add sort_order column & set correct ordering for existing DBs
  {
    ...
    const newAgents: [string, string, string, string, string, string, string][] = [
      ...
      ["Frida Kahlo", "프리다 칼로", "design", "junior", "gemini", "🌺", "감성적이고 독창적인 예술가"],
      ["Steve Jobs", "스티브 잡스", "design", "senior", "claude", "🍏", "혁신적인 제품 디자인 비전가"],
      ["Machiavelli", "마키아벨리", "planning", "senior", "claude", "📜", "군주론의 저자, 냉철한 전략 분석가"],
      ["James Watt", "제임스 와트", "operations", "senior", "codex", "🔧", "산업혁명의 아버지, 자동화의 선구자"],
      ["Bill Gates", "빌 게이츠", "operations", "senior", "codex", "💼", "글로벌 소프트웨어 제국 운영의 달인"],
      ["Marie Curie", "마리 퀴리", "qa", "team_leader", "claude", "⚗️", "노벨상 2회 수상, 철저한 실험과 검증의 대가"],
      ["Isaac Newton", "아이작 뉴턴", "qa", "senior", "opencode", "🍎", "자연법칙의 발견자, 정밀한 분석과 검증의 거장"],
      ["Sun Tzu", "손자", "devsecops", "team_leader", "claude", "⚔️", "손자병법의 저자, 최고의 방어 전략가"],
      ["Hedy Lamarr", "헤디 라마", "devsecops", "senior", "codex", "📡", "주파수 도약 기술 발명가, 보안 통신의 선구자"],
      ["Andrej Karpathy", "안드레이 카파시", "dev", "senior", "claude", "🤖", "AI/딥러닝 개발의 선구자"],
      ["Galileo Galilei", "갈릴레오 갈릴레이", "qa", "junior", "gemini", "🔭", "모든 것을 의심하고 관찰하는 탐구자"],
    ];
