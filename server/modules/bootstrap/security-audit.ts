...
export function createSecurityAuditTools(deps: SecurityAuditDeps) {
  ...
  const SECURITY_AUDIT_CHAIN_SEED = process.env.SECURITY_AUDIT_CHAIN_SEED?.trim() || "agentdesk-security-audit-v1";
  ...
  function appendSecurityAuditFallbackLog(payload: unknown): boolean {
    ...
    try {
      ...
      try {
        process.stderr.write(`[AgentDesk] security audit fallback append failed: ${String(fallbackErr)}\n${line}`);
