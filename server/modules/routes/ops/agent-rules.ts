...
const VALID_CATEGORIES = ["coding", "communication", "quality", "execution", "security", "workflow", "general"] as const;
...
function toPublic(row: AgentRuleRow) {
  return { ...row, enabled: !!row.enabled };
}

interface RegisterAgentRulesRoutesOptions {
  app: Express;
  db: DatabaseSync;
  nowMs: () => number;
}

export function registerAgentRulesRoutes({ app, db, nowMs }: RegisterAgentRulesRoutesOptions): void {
  // GET /api/agent-rules — list with optional filters
  app.get("/api/agent-rules", (_req, res) => {
    try {
      ...
      const sql = `
        SELECT ar.*,
          CASE
            WHEN ar.scope_type = 'department' THEN (SELECT d.name FROM departments d WHERE d.id = ar.scope_id)
            WHEN ar.scope_type = 'agent' THEN (SELECT a.name FROM agents a WHERE a.id = ar.scope_id)
            WHEN ar.scope_type = 'workflow_pack' THEN (SELECT w.name FROM workflow_packs w WHERE w.key = ar.scope_id)
            WHEN ar.scope_type = 'project' THEN (SELECT p.name FROM projects p WHERE p.id = ar.scope_id)
            ELSE NULL
          END AS scope_label
        FROM agent_rules ar
        ${where}
      ...
      const rows = db.prepare(sql).all(...params) as (AgentRuleRow & { scope_label?: string })[];
  ...
  // GET /api/agent-rules/:id — single rule
  app.get("/api/agent-rules/:id", (req, res) => {
    try {
      const row = db.prepare("SELECT * FROM agent_rules WHERE id = ?").get(req.params.id) as AgentRuleRow | undefined;
  ...
  // POST /api/agent-rules — create
  app.post("/api/agent-rules", (req, res) => {
    try {
      const body = req.body ?? {};
      const title = String(body.title ?? "").trim();
      const ruleContent = String(body.rule_content ?? "").trim();

      if (!title) return res.status(400).json({ error: "title required" });
      if (!ruleContent) return res.status(400).json({ error: "rule_content required" });
      ...
      const row = db.prepare("SELECT * FROM agent_rules WHERE id = ?").get(id) as AgentRuleRow;
  ...
  // PATCH /api/agent-rules/:id — update
  app.patch("/api/agent-rules/:id", (req, res) => {
    try {
      const existing = db.prepare("SELECT * FROM agent_rules WHERE id = ?").get(req.params.id) as
        | AgentRuleRow
        | undefined;
      ...
      const stringFields = ["title", "title_ko", "title_ja", "title_zh", "description", "rule_content"] as const;
  ...
  // PATCH /api/agent-rules/:id/toggle — toggle enabled
  app.patch("/api/agent-rules/:id/toggle", (req, res) => {
    try {
      const row = db.prepare("SELECT * FROM agent_rules WHERE id = ?").get(req.params.id) as
        | AgentRuleRow
        | undefined;
      if (!row) return res.status(404).json({ error: "not_found" });
  ...
  // PATCH /api/agent-rules/reorder — bulk priority update
  app.patch("/api/agent-rules/reorder", (req, res) => {
    try {
      ...
      for (const item of items) {
        if (item?.id && typeof item.priority === "number") {
          stmt.run(Math.max(1, Math.min(100, item.priority)), now, item.id);
        }
      }
  ...
  app.delete("/api/agent-rules/:id", (req, res) => {
    try {
      const row = db.prepare("SELECT id FROM agent_rules WHERE id = ?").get(req.params.id) as
        | { id: string }
        | undefined;
      if (!row) return res.status(404).json({ error: "not_found" });
