...
export function buildWorktreeCeoNote(
  ...
  return deps.pickL(
    deps.l(
      [` (격리 브랜치: agentdesk/${delegatedTaskId.slice(0, 8)})`],
      [` (isolated branch: agentdesk/${delegatedTaskId.slice(0, 8)})`],
      [` (分離ブランチ: agentdesk/${delegatedTaskId.slice(0, 8)})`],
      [`（隔离分支: agentdesk/${delegatedTaskId.slice(0, 8)}）`],
    ),
    lang,
  );
}
