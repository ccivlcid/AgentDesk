...
export function createProjectRouteHelpers({ db, normalizeTextField }: CreateProjectRouteHelpersOptions) {
  ...
  async function pickNativeDirectoryPathWindows(
    ...
    const psCommand = [
      ...
      "$folder = $shell.BrowseForFolder(0, 'Select project folder for AgentDesk', 65, 0);",
  ...
  async function pickNativeDirectoryPathWindowsVbs(
    ...
    const vbsScript = [
      ...
      'Set folder = shell.BrowseForFolder(0, "Select project folder for AgentDesk", 65, 0)',
  ...
  async function pickNativeDirectoryPath(): Promise<{ path: string | null; cancelled: boolean; source: string }> {
    ...
    if (process.platform === "darwin") {
      const script =
        'try\nPOSIX path of (choose folder with prompt "Select project folder for AgentDesk")\non error number -128\n""\nend try';
    ...
    try {
      const { stdout } = await execFileText(
        "zenity",
        ["--file-selection", "--directory", "--title=Select project folder for AgentDesk"],
        timeoutMs,
      );
      const value = stdout.trim();
      ...
      try {
        const { stdout } = await execFileText(
          ...
          [
            ...
            "--title",
            "Select project folder for AgentDesk",
          ],
          timeoutMs,
        );
        const value = stdout.trim();
