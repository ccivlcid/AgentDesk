...
export function createWorktreeLifecycleTools(deps: CreateWorktreeLifecycleToolsDeps) {
  ...
  function ensureWorktreeBootstrapRepo(projectPath: string, taskId: string): boolean {
    ...
    try {
      ...
      const baseIgnore = ["node_modules/", "dist/", ".agentdesk-worktrees/", ".agentdesk/", ".DS_Store", "*.log"];
      let existingExclude = "";
      try {
      ...
      console.error(`[AgentDesk] Failed git bootstrap for task ${shortId}: ${msg}`);
      return false;
    }
  }
  ...
  function createWorktree(projectPath: string, taskId: string, agentName: string, baseBranch?: string): string | null {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`[AgentDesk] Failed to create worktree for task ${shortId}: ${msg}`);
      return null;
    }
  }
