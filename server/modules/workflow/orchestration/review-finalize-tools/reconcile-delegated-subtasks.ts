...
export type FinishReviewFn = (
  taskId: string,
  taskTitle: string,
  options?: { bypassProjectDecisionGate?: boolean; trigger?: string },
) => void;

export function createReconcileDelegatedSubtasksAfterRun(
  deps: CreateReviewFinalizeToolsDeps,
  getFinishReview: () => FinishReviewFn,
): (taskId: string, exitCode: number) => void {
  const { db, nowMs, broadcast, appendTaskLog, getPreferredLanguage, pickL, l } = deps;

  return function reconcileDelegatedSubtasksAfterRun(taskId: string, exitCode: number): void {
    const linked = db
      .prepare(
        `
  SELECT id, task_id
