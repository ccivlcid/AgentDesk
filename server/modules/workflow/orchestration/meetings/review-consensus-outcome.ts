...
export async function processReviewConsensusOutcome(ctx: OutcomeContext): Promise<boolean> {
  ...
  if (needsRevision) {
    ...
    if ((isRound1Remediation || isRound2Merge) && !remediationLimitReached) {
      ...
      notifyClient(
        pickL(
          l(
            [
              `[Client OFFICE] '${taskTitle}' 리뷰 라운드 ${round}에서 팀장 보완 의견이 취합되었습니다. 의사결정 인박스에서 항목을 복수 선택(체리피킹)하고 필요 시 추가 의견을 입력해 보완 작업을 진행하거나, 다음 라운드(${nextRound})로 SKIP할지 선택해 주세요.`,
            ],
            [
              `[Client OFFICE] Team-lead remediation opinions for '${taskTitle}' in review round ${round} are consolidated. In Decision Inbox, cherry-pick multiple items and optionally add an extra note for remediation, or skip to round ${nextRound}.`,
            ],
            [
              `[Client OFFICE] '${taskTitle}' のレビューラウンド${round}でチームリーダー補完意見を集約しました。Decision Inboxで複数項目をチェリーピックし、必要に応じて追加意見を入力して補完実行するか、ラウンド${nextRound}へスキップするか選択してください。`,
            ],
            [
              `[Client OFFICE] '${taskTitle}' 第${round}轮已汇总组长整改意见。请在 Decision Inbox 中多选条目并可追加补充意见后执行整改，或直接跳到第 ${nextRound} 轮。`,
            ],
          ),
          lang,
        ),
    ...
    if ((isRound1Remediation || isRound2Merge) && remediationLimitReached) {
      ...
      notifyClient(
        pickL(
          l(
            [
              `[Client OFFICE] '${taskTitle}' 보완 요청은 태스크당 최대 ${REVIEW_MAX_REMEDIATION_REQUESTS}회 정책에 따라 추가 보완 생성 없이 최종 판단 단계로 전환합니다.`,
            ],
            [
              `[Client OFFICE] '${taskTitle}' reached the remediation-request cap (${REVIEW_MAX_REMEDIATION_REQUESTS} per task). Skipping additional remediation and moving to final decision.`,
            ],
            [
              `[Client OFFICE] '${taskTitle}' はタスク当たり補完要請上限（${REVIEW_MAX_REMEDIATION_REQUESTS}回）に到達したため、追加補完を作成せず最終判断へ移行します。`,
            ],
            [
              `[Client OFFICE] '${taskTitle}' 已达到每个任务最多 ${REVIEW_MAX_REMEDIATION_REQUESTS} 次整改请求上限，不再新增整改，转入最终判断。`,
            ],
          ),
          lang,
        ),
    ...
    notifyClient(
      pickL(
        l(
          [
            `[Client OFFICE] '${taskTitle}' 리뷰 라운드 ${round}에서 잔여 리스크를 최종 문서에 반영했습니다. 추가 보완 없이 최종 승인 판단으로 종료합니다.`,
          ],
          [
            `[Client OFFICE] In review round ${round} for '${taskTitle}', residual risks were embedded in the final document package. Closing with final approval decision and no further remediation.`,
          ],
          [
            `[Client OFFICE] '${taskTitle}' のレビューラウンド${round}で残余リスクを最終文書へ反映しました。追加補完なしで最終承認判断を完了します。`,
          ],
          [
            `[Client OFFICE] '${taskTitle}' 第${round}轮评审已将剩余风险写入最终文档包，在不新增整改的前提下完成最终审批判断。`,
          ],
        ),
        lang,
      ),
  ...
  if (deferredMonitoringLeaders.length > 0) {
    notifyClient(
      pickL(
        l(
          [
            `[Client OFFICE] '${taskTitle}' 리뷰 라운드 ${round}에서 ${deferredMonitoringLeaders.length}개 보류 의견이 'MVP 범위 외 항목의 SLA 모니터링 전환'으로 분류되어 코드 병합 후 후속 체크리스트로 이관합니다.`,
          ],
          [
            `[Client OFFICE] In review round ${round} for '${taskTitle}', ${deferredMonitoringLeaders.length} hold opinions were classified as MVP-out-of-scope and moved to post-merge SLA monitoring checklist.`,
          ],
          [
            `[Client OFFICE] '${taskTitle}' のレビューラウンド${round}では、保留意見${deferredMonitoringLeaders.length}件を「MVP範囲外のSLA監視項目」へ振替し、コード統合後のチェックリストで追跡します。`,
          ],
          [
            `[Client OFFICE] '${taskTitle}' 第${round}轮 Review 中，有 ${deferredMonitoringLeaders.length} 条保留意见被判定为 MVP 范围外事项，已转入合并后的 SLA 监控清单跟踪。`,
          ],
        ),
        lang,
      ),
  ...
  if (isRound2Merge) {
    ...
    notifyClient(
      pickL(
        l(
          [
            `[Client OFFICE] '${taskTitle}' 리뷰 라운드 ${round} 취합/머지 검토가 완료되었습니다. 라운드 3 최종 승인 회의로 전환합니다.`,
          ],
          [
            `[Client OFFICE] Review round ${round} consolidation/merge review for '${taskTitle}' is complete. Moving to round 3 final approval.`,
          ],
          [
            `[Client OFFICE] '${taskTitle}' のレビューラウンド${round}集約/マージ確認が完了しました。ラウンド3最終承認へ移行します。`,
          ],
          [`[Client OFFICE] '${taskTitle}' 第${round}轮评审汇总/合并审查已完成，现转入第3轮最终审批。`],
        ),
        lang,
      ),
      taskId,
  ...
  notifyClient(
    pickL(
      l(
        [`[Client OFFICE] '${taskTitle}' 전원 Approved 완료. Done 단계로 진행합니다.`],
        [`[Client OFFICE] '${taskTitle}' is approved by all leaders. Proceeding to Done.`],
        [`[Client OFFICE] '${taskTitle}' は全リーダー承認済みです。Doneへ進みます。`],
        [`[Client OFFICE] '${taskTitle}'已获全体负责人批准，进入 Done 阶段。`],
      ),
      lang,
    ),
    taskId,
