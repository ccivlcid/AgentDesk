...
export function createUsageCliTools(deps: CreateUsageCliToolsDeps) {
  ...
  // Map CLI binary name to provider key (most are 1:1, except "agent" → "cursor")
  const CLI_NAME_TO_PROVIDER: Record<string, string> = { agent: "cursor" };
  ...
  async function detectAllCli(): Promise<CliStatusResult> {
    ...
    for (let i = 0; i < CLI_TOOLS.length; i++) {
      const key = CLI_NAME_TO_PROVIDER[CLI_TOOLS[i].name] ?? CLI_TOOLS[i].name;
      out[key] = results[i];
    }
    return out;
  }
