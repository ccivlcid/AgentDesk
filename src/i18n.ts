...
export const LANGUAGE_USER_SET_STORAGE_KEY = "agentdesk.language.user_set";
...
export function useI18n(languageOverride?: string | null): I18nContextValue {
  ...
  useEffect(() => {
    ...
    window.addEventListener("agentdesk-language-change", sync as EventListener);
    return () => {
      window.removeEventListener("storage", sync);
      window.removeEventListener("agentdesk-language-change", sync as EventListener);
    };
