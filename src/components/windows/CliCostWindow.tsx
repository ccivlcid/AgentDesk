...
export default function CliCostWindow() {
  ...
  return (
    <AppWindow
      ...
      title={t({ ko: "CLI 사용량", en: "CLI Usage", ja: "CLI使用量", zh: "CLI用量" })}
    ...
    >
      <div style={{ padding: 24, fontFamily: mono, fontSize: 12, overflowY: "auto", height: "100%", background: "var(--th-bg-primary)" }}>
        {loading ? (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "var(--th-text-muted)" }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--th-accent)" strokeWidth="2" style={{ animation: "spin 1s linear infinite", marginRight: 8 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", color: "var(--th-text-muted)", gap: 10 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--th-text-secondary)" }}>{t({ ko: "비용 데이터 없음", en: "No cost data", ja: "コストデータなし", zh: "无成本数据" })}</span>
            ...
            {Object.entries(usage).map(([provider, entry]) => (
              <div key={provider} style={{ border: "1px solid var(--th-border)", borderRadius: 16, overflow: "hidden", background: "var(--th-bg-elevated)" }}>
                ...
                {entry.windows.length === 0 && !entry.error ? (
                  <div style={{ padding: "14px 18px", color: "var(--th-text-muted)", fontSize: 11 }}>
                  ...
                  <div style={{ padding: "12px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
                    {entry.windows.map((w, i) => {
                      ...
                      const barColor = pct > 80 ? "var(--th-danger-text)" : pct > 50 ? "#D97706" : "var(--th-success)";
                      const barBg = pct > 80 ? "var(--th-danger-bg)" : pct > 50 ? "#FFFBEB" : "#ECFDF5";
                      return (
                        <div key={i}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                            <span style={{ fontSize: 11, color: "var(--th-text-secondary)", fontWeight: 500 }}>{w.label}</span>
