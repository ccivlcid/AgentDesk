...
import type { MemoryCategory, MemoryScopeType, Agent, AgentRole } from "../../types";
import type { MemoryHistoryProvider, MemoryLearnProvider, MemoryLearnJob } from "../../api/memory";
...
export const ALL_CATEGORIES = ["All", ...MEMORY_CATEGORIES] as const;
...
export const CATEGORY_COLORS: Record<string, string> = {
  ...
  instruction: "text-cyan-400 bg-cyan-500/15 border-cyan-500/30",
  reference: "text-rose-400 bg-rose-500/15 border-rose-500/30",
};

export function categoryLabel(category: string, t: TFunction): string {
  const labels: Record<string, Record<Locale, string>> = {
    All: { ko: "\uC804\uCCB4", en: "All", ja: "\u3059\u3079\u3066", zh: "\u5168\u90E8" },
    context: { ko: "\uCEE8\uD14D\uC2A4\uD2B8", en: "Context", ja: "\u30B3\u30F3\u30C6\u30AD\u30B9\u30C8", zh: "\u4E0A\u4E0B\u6587" },
    preference: { ko: "\uD658\uACBD\uC124\uC815", en: "Preference", ja: "\u8A2D\u5B9A", zh: "\u504F\u597D" },
    convention: { ko: "\uADDC\uCE59", en: "Convention", ja: "\u898F\u7D04", zh: "\u60EF\u4F8B" },
    knowledge: { ko: "\uC9C0\uC2DD", en: "Knowledge", ja: "\u77E5\u8B58", zh: "\u77E5\u8BC6" },
    instruction: { ko: "\uC9C0\uC2DC", en: "Instruction", ja: "\u6307\u793A", zh: "\u6307\u4EE4" },
    reference: { ko: "\uCC38\uC870", en: "Reference", ja: "\u53C2\u7167", zh: "\u5F15\u7528" },
  };
  return labels[category] ? t(labels[category]) : category;
}

export const SCOPE_TYPES: MemoryScopeType[] = ["global", "department", "agent", "workflow_pack"];
...
export type MemorySortBy = "priority" | "name" | "date";

// ── Memory Learning constants & utilities ─────────────────────────────

export const MEMORY_LEARN_PROVIDER_ORDER: MemoryLearnProvider[] = ["claude", "codex", "gemini", "opencode", "cursor"];
export const MEMORY_LEARNED_PROVIDER_ORDER: MemoryHistoryProvider[] = [
...
export function memoryProviderLabel(provider: MemoryLearnProvider | MemoryHistoryProvider): string {
...
export function memoryLearningRowKey(row: { provider: MemoryHistoryProvider; memory_id: string }): string {
  return `${row.provider}:${row.memory_id}`;
}

export function learningStatusLabel(status: MemoryLearnJob["status"] | null, t: TFunction): string {
...
export function cliProviderIcon(provider: MemoryHistoryProvider) {
