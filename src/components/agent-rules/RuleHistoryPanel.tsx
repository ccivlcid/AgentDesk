  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 px-3 py-2.5" style={{ borderBottom: "1px solid #E5E7EB" }}>
        <div className="flex items-center gap-1">
          <button
            ...
            style={{
              borderRadius: 0,
              border: `1px solid ${tab === "history" ? "#D1D5DB" : "transparent"}`,
              background: tab === "history" ? "#F9FAFB" : "transparent",
          ...
          <button
            ...
            style={{
              borderRadius: 0,
              border: `1px solid ${tab === "available" ? "#D1D5DB" : "transparent"}`,
              background: tab === "available" ? "#F9FAFB" : "transparent",
              color: tab === "available" ? "#111827" : "#9CA3AF",
            }}
          >
      ...
      <div className="flex items-center gap-2 px-3 py-2" style={{ borderBottom: "1px solid #E5E7EB" }}>
        <div className="relative" ref={filterDropdownRef}>
          <button
            ...
            style={{
              ...
              background: agentFilters.size > 0 ? "rgba(251,191,36,0.08)" : "#F3F4F6",
              color: agentFilters.size > 0 ? "#3B82F6" : "#6B7280",
              minWidth: 130,
            }}
          >
          ...
          {filterDropdownOpen && (
            <div
              ...
              style={{ borderRadius: 0, border: "1px solid #D1D5DB", background: "#FFFFFF", minWidth: 200, maxHeight: 240, overflowY: "auto" }}
            >
              <button
                type="button"
                onClick={clearAgentFilters}
                className="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] font-mono text-left"
                style={{ background: agentFilters.size === 0 ? "rgba(251,191,36,0.1)" : "transparent", color: agentFilters.size === 0 ? "#3B82F6" : "#6B7280", border: "none" }}
              >
                <span className="flex items-center justify-center shrink-0" style={{ width: 14, height: 14, borderRadius: 0, border: agentFilters.size === 0 ? "1px solid #3B82F6" : "1px solid #D1D5DB", background: agentFilters.size === 0 ? "#3B82F6" : "transparent", fontSize: 9, color: "#000" }}>
                  {agentFilters.size === 0 ? (
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
              ...
              {agentsWithProvider.map((agent) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] font-mono text-left"
                    style={{ background: checked ? "rgba(251,191,36,0.08)" : "transparent", color: checked ? "#3B82F6" : "#6B7280", border: "none" }}
                  >
                    <span className="flex items-center justify-center shrink-0" style={{ width: 14, height: 14, borderRadius: 0, border: checked ? "1px solid #3B82F6" : "1px solid #D1D5DB", background: checked ? "#3B82F6" : "transparent", fontSize: 9, color: "#000" }}>
                      {checked ? (
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                          <polyline points="20 6 9 17 4 12" />
                        </svg>
        ...
        {agentFilters.size > 0 && (
          <button type="button" onClick={clearAgentFilters} className="px-1.5 py-0.5 text-[10px] font-mono inline-flex items-center gap-1" style={{ borderRadius: 0, border: "1px solid #E5E7EB", color: "#9CA3AF", background: "transparent" }}>
      ...
      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto px-3 pb-3">
        {loading && historyRows.length === 0 && availableRows.length === 0 && (
          <div className="px-3 py-6 text-center text-xs font-mono" style={{ borderRadius: 0, border: "1px solid #E5E7EB", background: "#FFFFFF", color: "#9CA3AF" }}>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="truncate text-xs font-semibold font-mono" style={{ color: "#111827" }}>{label}</div>
                    <div className="mt-0.5 truncate text-[10px] font-mono" style={{ color: "#9CA3AF" }}>{row.rule_id}</div>
                  </div>
                ...
                <div className="skill-history-meta mt-2 flex items-center justify-between gap-2 text-[10px] font-mono" style={{ color: "#9CA3AF" }}>
                  <div className="flex min-w-0 items-center gap-2">
                    {rowAgents.length > 0 ? (
                      <div className={`flex items-center gap-0.5 ${unlearnEffect ? "unlearn-avatar-hit" : ""}`}>
                        {rowAgents.slice(0, 5).map((a) => (
                          <div key={a.id} className="h-5 w-5 overflow-hidden shrink-0" style={{ borderRadius: 0, background: "#F3F4F6" }}>
                            <AgentAvatar agent={a} agents={agents} size={20} rounded="xl" />
                          </div>
                        ))}
                        {rowAgents.length > 5 && <span className="text-[9px] font-mono ml-0.5" style={{ color: "#9CA3AF" }}>+{rowAgents.length - 5}</span>}
                      ...
                      <div className={`relative h-5 w-5 overflow-hidden ${unlearnEffect ? "unlearn-avatar-hit" : ""}`} style={{ borderRadius: 0, background: "#F3F4F6" }}>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    {canUnlearn && (
                      <button
                        ...
                        style={{
                          ...
                          color: isUnlearning ? "#9CA3AF" : "rgb(253,164,175)",
                    ...
                    <span className="skill-history-time" style={{ color: "#9CA3AF" }}>{ruleRelativeTime(eventAt, localeTag)}</span>
                  </div>
                </div>
          ...
          filteredAvailableRows.map((row) => {
            ...
            return (
              ...
              >
                <div className="truncate text-xs font-semibold font-mono" style={{ color: "#111827" }}>{label}</div>
                <div className="mt-0.5 truncate text-[10px] font-mono" style={{ color: "#9CA3AF" }}>{row.rule_id}</div>
                <div className="skill-history-meta mt-2 flex items-center justify-between gap-2 text-[10px] font-mono" style={{ color: "#6B7280" }}>
                  <div className="flex min-w-0 items-center gap-2">
                    {rowAgents.length > 0 ? (
                      <div className={`flex items-center gap-0.5 ${unlearnEffect ? "unlearn-avatar-hit" : ""}`}>
                        {rowAgents.slice(0, 5).map((a) => (
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <span className="skill-history-time font-mono" style={{ color: "#9CA3AF" }}>{ruleRelativeTime(row.learned_at, localeTag)}</span>
                  </div>
                </div>
              </div>
            );
      ...
      {centerBonk && (
        <div className="pointer-events-none fixed inset-0 z-[120] flex items-center justify-center">
          <div className="skill-history-center-card unlearn-center-card px-6 py-4" style={{ borderRadius: 0, border: "1px solid rgba(251,113,133,0.3)", background: "#1E1E2E" }}>
