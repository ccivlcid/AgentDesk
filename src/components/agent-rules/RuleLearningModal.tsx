...
interface RuleLearningModalProps {
  t: TFunction;
  localeTag: string;
  agents: Agent[];
  learningRule: AgentRule | null;
  learnInProgress: boolean;
  selectedProviders: RuleLearnProvider[];
  representatives: Array<{ provider: RuleLearnProvider; agent: Agent | null }>;
  preferKoreanName: boolean;
  modalLearnedProviders: Set<RuleHistoryProvider>;
  unlearningProviders: RuleLearnProvider[];
  unlearnEffects: Partial<Record<RuleLearnProvider, UnlearnEffect>>;
  learnJob: {
    id: string;
  ...
  defaultSelectedProviders: RuleLearnProvider[];
  onClose: () => void;
  onToggleProvider: (provider: RuleLearnProvider) => void;
  onUnlearnProvider: (provider: RuleLearnProvider) => void;
  onStartLearning: () => void;
}

export default function RuleLearningModal({
  t,
  localeTag,
  agents,
  learningRule,
        ...
        <div className="space-y-4 overflow-y-auto px-5 py-4 max-h-[calc(90vh-72px)]">
          ...
          <div className="rounded-xl border border-emerald-500/25 bg-slate-950/60 px-3 py-2">
            ...
            <div className="mt-1 text-[11px] font-mono text-green-300 max-h-20 overflow-y-auto whitespace-pre-wrap break-all">
              {learningRule.rule_content.slice(0, 500)}
              {learningRule.rule_content.length > 500 ? "..." : ""}
            </div>
          </div>
