...
interface KeyEntry { keys: string[]; desc: string }
interface Callout { type: "tip" | "warn" | "info"; text: string }
interface Section {
  heading: string;
  body?: string;
  keys?: KeyEntry[];
      ...
      sections: [
        {
          ...
          body: t({
            ...
            ja: "Settings → APIタブでキーを登録します。",
            zh: "设置(g s) → API选项卡，注册各主要模型提供商的API密钥。",
          }),
        },
        {
          heading: t({ ko: "OAuth / 채널 연동", en: "OAuth / Channel Integration", ja: "OAuth / チャンネル連携", zh: "OAuth / 频道集成" }),
          body: t({
            ko: "Settings → OAuth 탭\n• Discord · Slack 채널과 AgentDesk를 연결합니다\n• 에이전트 알림·메시지를 외부 채널로 발송합니다\n• 채널 가이드 아이콘(?) 클릭으로 설정 방법 확인",
            ...
            ja: "Settings → OAuthタブでDiscord/Slackと連携します。",
            zh: "设置 → OAuth选项卡连接Discord或Slack频道。",
          }),
        },
        {
          heading: t({ ko: "데이터 내보내기", en: "Data Export", ja: "データエクスポート", zh: "数据导出" }),
          body: t({
            ko: "앱 메뉴(AgentDesk 클릭) → '내보내기...'\n• 에이전트·태스크·워크플로 데이터를 CSV/JSON으로 내보냅니다\n• 날짜 범위와 데이터 유형을 선택하여 필요한 부분만 추출합니다",
            ...
            ja: "アプリメニュー → 「エクスポート...」でCSV/JSONとして書き出します。",
            zh: "应用菜单 → 「导出...」将数据导出为CSV或JSON格式。",
          }),
        },
      ],
    },
      ...
      sections: [
        {
          ...
          body: t({
            ko: "Image Studio는 AI 이미지 생성 도구입니다.\n바탕화면 Image Studio 아이콘 클릭 또는 단축키 g i 로 창을 열 수 있습니다.\n\n• Generate 탭 — 프롬프트로 이미지 생성\n• Gallery 탭 — 생성된 이미지 관리·저장·삭제",
            en: "Image Studio is an AI image generation tool.\nOpen it by clicking the Image Studio icon on the desktop or pressing g i.\n\n• Generate tab — create images from text prompts\n• Gallery tab — manage, save, and delete generated images",
            ja: "Image StudioはAI画像生成ツールです。デスクトップアイコンまたはg iで開きます。\n• Generateタブ — プロンプトから画像生成\n• Galleryタブ — 生成画像の管理",
            zh: "图像工作室是AI图像生成工具。点击桌面图标或按g i打开。\n• 生成选项卡 — 从文本提示生成图像\n• 画廊选项卡 — 管理、保存和删除生成的图像",
          }),
        },
        {
          heading: t({ ko: "사전 준비 — 이미지 생성 프로바이더 추가", en: "Setup — Add an Image Generation Provider", ja: "準備 — 画像生成プロバイダを追加", zh: "准备 — 添加图像生成提供商" }),
          body: t({
            ko: "Image Studio는 Settings → API 제공자에 등록된 프로바이더를 사용합니다.\nClaude / Ollama 같은 텍스트 전용 모델은 이미지 생성이 불가합니다.\n\n이미지 생성이 가능한 프로바이더:\n• OpenAI — DALL-E 3, DALL-E 2 (가장 권장)\n• Azure OpenAI — DALL-E 3\n• Together.ai — Flux, Stable Diffusion XL\n• 로컬 Stable Diffusion — AUTOMATIC1111 또는 ComfyUI\n\n등록 방법:\n1. Settings(g s) → API 제공자 탭\n2. '+ 추가' → 프로바이더 유형·이름·API 키·Base URL 입력\n3. '연결 테스트' 클릭 → 모델 목록 자동 캐시\n4. Image Studio를 열면 해당 프로바이더가 자동으로 표시됩니다",
            en: "Image Studio uses providers registered in Settings → API Providers.\nText-only models like Claude or Ollama cannot generate images.\n\nSupported image generation providers:\n• OpenAI — DALL-E 3, DALL-E 2 (recommended)\n• Azure OpenAI — DALL-E 3\n• Together.ai — Flux, Stable Diffusion XL\n• Local Stable Diffusion — AUTOMATIC1111 or ComfyUI\n\nHow to add:\n1. Open Settings (g s) → API Providers tab\n2. Click '+ Add' → enter provider type, name, API key, Base URL\n3. Click 'Test Connection' → models are cached automatically\n4. Open Image Studio — the provider appears in the dropdown",
            ja: "Image StudioはSettings → APIプロバイダに登録されたプロバイダを使用します。\nClaudeやOllamaなどのテキスト専用モデルは画像生成不可です。\n\n対応プロバイダ:\n• OpenAI — DALL-E 3, DALL-E 2\n• Azure OpenAI — DALL-E 3\n• Together.ai — Flux, SD XL\n• ローカルStable Diffusion\n\n追加方法: Settings(g s) → APIプロバイダ → +追加 → 接続テスト",
            zh: "图像工作室使用Settings → API提供商中注册的提供商。\nClaude、Ollama等纯文本模型无法生成图像。\n\n支持的提供商:\n• OpenAI — DALL-E 3, DALL-E 2（推荐）\n• Azure OpenAI — DALL-E 3\n• Together.ai — Flux, SD XL\n• 本地Stable Diffusion\n\n添加方法: Settings(g s) → API提供商 → +添加 → 测试连接",
          }),
          callout: {
            type: "tip",
            text: t({
              ko: "Together.ai는 무료 크레딧을 제공하며 Flux 모델로 고품질 이미지를 생성할 수 있습니다. OpenAI 계정 없이도 시작할 수 있습니다.",
              en: "Together.ai offers free credits and supports Flux models for high-quality images — no OpenAI account needed to get started.",
              ja: "Together.aiは無料クレジットを提供しており、OpenAIアカウントなしでFluxモデルで高品質な画像を生成できます。",
              zh: "Together.ai提供免费积分，支持Flux模型生成高质量图像——无需OpenAI账号即可开始使用。",
            }),
          },
        },
        {
          heading: t({ ko: "이미지 생성하기", en: "Generating Images", ja: "画像の生成", zh: "生成图像" }),
          body: t({
            ko: "1. Generate 탭의 좌측 패널에 프롬프트 입력\n2. 제공자·모델·크기 선택\n3. ▶ 생성 버튼 클릭 또는 Ctrl+Enter\n4. 중앙 캔버스에 결과 이미지가 표시됩니다\n\n옵션 (DALL-E 3 기준):\n• 크기 — 1024×1024 / 1024×1792 / 1792×1024\n• 품질 — Standard / HD\n• 스타일 — Vivid (선명) / Natural (자연스러운)",
            en: "1. Enter a prompt in the left panel of the Generate tab\n2. Select provider, model, and size\n3. Click ▶ Generate or press Ctrl+Enter\n4. The result appears in the center canvas\n\nOptions (DALL-E 3):\n• Size — 1024×1024 / 1024×1792 / 1792×1024\n• Quality — Standard / HD\n• Style — Vivid / Natural",
            ja: "1. Generateタブの左パネルにプロンプトを入力\n2. プロバイダ・モデル・サイズを選択\n3. ▶生成またはCtrl+Enter\n4. 中央キャンバスに結果が表示されます",
            zh: "1. 在Generate选项卡的左侧面板输入提示词\n2. 选择提供商、模型和尺寸\n3. 点击▶生成或按Ctrl+Enter\n4. 结果显示在中央画布上",
          }),
        },
        {
          heading: t({ ko: "이미지 저장하기", en: "Saving Images", ja: "画像の保存", zh: "保存图像" }),
          body: t({
            ko: "생성된 이미지는 서버에 자동으로 저장되며 Gallery 탭에서 언제든 다시 볼 수 있습니다.\n\n추가로 로컬에 저장하는 방법:\n• 이미지 우상단 ↓ 버튼 클릭 (즉시 다운로드)\n• 좌측 패널 Output 섹션 → '↓ 이미지 저장' 버튼\n• Gallery 탭에서 이미지 선택 → 우측 패널 '↓ 이미지 저장'\n• Gallery 썸네일 hover → ↓ 아이콘 클릭",
            en: "Generated images are automatically saved on the server and always accessible in the Gallery tab.\n\nTo download locally:\n• Click the ↓ button in the top-right corner of the image\n• Left panel Output section → '↓ Save Image' button\n• Gallery tab → select image → right panel '↓ Save Image'\n• Hover over a Gallery thumbnail → click ↓",
            ja: "生成画像はサーバーに自動保存され、Galleryタブでいつでも確認できます。\nローカル保存: 画像右上の↓ボタン / 左パネル / Galleryの右パネル",
            zh: "生成的图像自动保存在服务器上，随时可在画廊选项卡中查看。\n本地保存: 图像右上角↓按钮 / 左侧面板 / 画廊右侧面板",
          }),
        },
        {
          heading: t({ ko: "갤러리 관리", en: "Gallery Management", ja: "ギャラリー管理", zh: "画廊管理" }),
          body: t({
            ko: "• 프롬프트 검색으로 원하는 이미지를 빠르게 찾기\n• 이미지 클릭 → 우측 패널에서 상세 정보 확인 (프롬프트·모델·해상도·생성일)\n• 썸네일 hover → ↓ 저장 / ✕ 삭제\n• 삭제 시 서버 파일도 함께 제거됩니다",
            en: "• Search prompts to quickly find images\n• Click an image → view details in the right panel (prompt, model, resolution, date)\n• Hover a thumbnail → ↓ download / ✕ delete\n• Deleting also removes the file from the server",
            ja: "• プロンプト検索で素早く画像を検索\n• 画像クリック → 右パネルで詳細確認\n• サムネイルhover → ↓保存 / ✕削除",
            zh: "• 搜索提示词快速找到图像\n• 点击图像 → 在右侧面板查看详情\n• 悬停缩略图 → ↓下载 / ✕删除",
          }),
          callout: {
            type: "info",
            text: t({
              ko: "이미지는 AgentDesk 데이터 폴더(image-studio/)에 PNG로 저장되며, 앱을 재시작해도 유지됩니다.",
              en: "Images are stored as PNG files in the AgentDesk data folder (image-studio/) and persist across restarts.",
              ja: "画像はAgentDeskデータフォルダ(image-studio/)にPNGとして保存され、再起動後も保持されます。",
              zh: "图像以PNG格式存储在AgentDesk数据文件夹(image-studio/)中，重启后仍然保留。",
            }),
          },
        },
      ],
...
function CalloutBox({ type, text }: Callout) {
  const colors = {
...
function FeatureGrid({ features }: { features: { icon: string; label: string; desc: string }[] }) {
  return (
      ...
      {features.map((f) => (
        <div key={f.label} style={{
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontFamily: mono, fontSize: 11, fontWeight: 600, color: "var(--th-text-primary)", marginBottom: 2 }}>{f.label}</div>
            <div style={{ fontFamily: mono, fontSize: 10, color: "var(--th-text-muted)", lineHeight: 1.4 }}>{f.desc}</div>
          </div>
        </div>
      ))}
    </div>
...
export default function UserGuidePanel({ open, onClose, initialChapter }: UserGuidePanelProps) {
  ...
  // Search filter: match chapter titles + section headings/body
...
function ChapterBtn({ color, icon, title, active, onClick }: {
  color: string; icon: string; title: string; active: boolean; onClick: () => void;
}) {
