  ...
  return (
    <div className="border-b px-4 py-3 space-y-2" style={{ borderColor: "var(--th-border)" }}>
      ...
      <textarea
        ...
        value={prompt}
        onChange={(e) => {
          const next = e.target.value.slice(0, INTERVENTION_PROMPT_MAX_LENGTH);
          setPrompt(next);
        }}
        onKeyDown={(e) => {
      ...
      {message && <div className="text-[11px] text-emerald-300 break-words">{message}</div>}
      <div className="flex flex-wrap items-center gap-2">
        {taskStatus === "in_progress" && onPauseOnly && (
          <button
            type="button"
            onClick={() => void onPauseOnly()}
            disabled={busy}
        ...
        >
          {busy ? tr("실행 중...", "Running...", "実行中...", "执行中...") : tr("난입 실행", "Inject + Resume", "割込実行", "中断注入")}
        </button>
        {canInjectPrompt && onResumeOnly && (
          <button
            type="button"
            onClick={() => void onResumeOnly()}
            disabled={busy}
            className="px-2.5 py-1.5 text-[11px] font-mono border transition disabled:opacity-50"
            style={{
              borderRadius: 0,
