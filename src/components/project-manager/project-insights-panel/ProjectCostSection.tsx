  ...
  return (
    <div className="min-w-0 p-4" style={{ border: "1px solid #E5E7EB", borderRadius: 0, background: "#F9FAFB" }}>
      ...
      {loading ? (
        ...
        <p className="text-xs font-mono" style={{ color: "#9CA3AF" }}>-</p>
      ) : (
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div className="p-2" style={{ background: "#FFFFFF", border: "1px solid #E5E7EB" }}>
              <p className="text-[10px] font-mono uppercase" style={{ color: "#9CA3AF" }}>
            ...
            <div className="p-2" style={{ background: "#FFFFFF", border: "1px solid #E5E7EB" }}>
              ...
              <p className="mt-0.5 text-sm font-bold font-mono" style={{ color: "#111827" }}>{fmtUsd(cost.totalUsd)}</p>
            </div>
          </div>
          ...
          <div className="flex gap-3 text-[11px] font-mono">
            <span style={{ color: "#9CA3AF" }}>
              ...
              <span className="ml-1" style={{ color: "#6B7280" }}>{fmtTokens(cost.totalTokensIn)}</span>
            </span>
            <span style={{ color: "#9CA3AF" }}>
              {t({ ko: "출력", en: "Out", ja: "出力", zh: "输出" })}:
              <span className="ml-1" style={{ color: "#6B7280" }}>{fmtTokens(cost.totalTokensOut)}</span>
            </span>
          </div>

          {cost.agentBreakdown.length > 0 && (
            <div>
              <p className="mb-1.5 text-[11px] font-medium font-mono" style={{ color: "#9CA3AF" }}>
              ...
              <div className="space-y-1">
                {cost.agentBreakdown.slice(0, 5).map((row) => (
                  <div key={row.agentId} className="flex items-center gap-2 text-[11px] font-mono">
                    <span className="w-24 truncate" style={{ color: "#6B7280" }}>{row.agentName}</span>
                    <div className="flex-1 overflow-hidden" style={{ height: 5, background: "#F3F4F6" }}>
                      <div
                        className="h-full bg-amber-400/60"
                        style={{ width: cost.totalUsd > 0 ? `${Math.min(100, (row.totalUsd / cost.totalUsd) * 100)}%` : "0%" }}
                      />
                    </div>
                    <span className="w-16 text-right" style={{ color: "#9CA3AF" }}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {cost.packBreakdown.map((row) => (
                  ...
                  >
                    <span className="text-[10px] font-mono" style={{ color: "#9CA3AF" }}>{row.packKey}</span>
