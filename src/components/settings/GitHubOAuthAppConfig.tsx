...
export default function GitHubOAuthAppConfig({ t }: { t: TFunction }) {
  ...
  return (
    <div className="space-y-2 p-4" style={{ borderRadius: 8, border: "1px solid var(--th-border)", background: "var(--th-bg-surface)" }}>
      ...
      {ghClientIdLoaded && (
        <div className="flex gap-2 items-center">
          <input
            ...
            className="flex-1 px-3 py-1.5 text-xs font-mono outline-none" style={{ borderRadius: 8, border: "1px solid var(--th-border)", background: "var(--th-bg-elevated)", color: "var(--th-text-primary)" }}
          />
          <button
            onClick={saveClientId}
            className="shrink-0 px-3 py-1.5 text-[11px] font-mono font-medium transition" style={{ borderRadius: 8, background: "var(--th-accent)", color: "var(--th-bg-elevated)" }}
          >
