    ...
    <div className={`space-y-2 ${className}`}>
      {finalBlocks.map((block, bi) => {
        if (block.type === "code") {
          return (
            <pre
              ...
              style={{ borderRadius: 0, background: "var(--th-terminal-bg)", border: "1px solid var(--th-border)", color: "rgb(110,231,183)" }}
            >
              {block.content}
            </pre>
          );
        ...
        if (block.type === "table") {
          ...
          if (table) {
            return (
              <div key={bi} className="overflow-x-auto" style={{ borderRadius: 0, border: "1px solid var(--th-border)" }}>
